...
import { CdkStack } from "../lib/ad-connect";
...
import { DecryptSecretValue } from "../lib/tools/secret_decryption";
...
if (configFile && fs.existsSync(configFile)) {
  ...
  (async () => {
    const StackName = config.general.prefix.toLocaleUpperCase() +"-"+ config.general.produkt.toLocaleUpperCase() +"-"+ config.ad.ShortName.toLocaleUpperCase() + "-" + config.general.stage.toLocaleUpperCase()
    ...
    if (validate(config)) {
      const test = await DecryptSecretValue(runtimeprops,config,region)
      console.log(
